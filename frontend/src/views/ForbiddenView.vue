...
<template>
  <main class="error-page">
    <header class="error-nav">
      <router-link class="error-brand" to="/guest/home">
        <AppLogo class="error-brand-logo" :size="22" />
        <span>社区居住服务</span>
      </router-link>
      <nav class="error-nav-links">
        <router-link to="/guest/home">首页</router-link>
        <router-link to="/auth/login">登录</router-link>
      </nav>
    </header>

    <section class="error-body">
      <img class="error-illustration" src="/images/error-403.png" alt="锁与盾牌插画" />
      <h1 class="error-code">403</h1>
      <h2 class="error-title">无权访问该页面</h2>
      <p class="error-desc">你当前的账号没有访问此页面的权限，如有疑问请联系社区管理员</p>
      <div class="error-actions">
        <el-button type="primary" class="error-btn error-btn--primary" @click="backHome">
          返回首页
        </el-button>
        <el-button class="error-btn error-btn--ghost" @click="reLogin">重新登录</el-button>
      </div>
    </section>
  </main>
</template>
...
/* 插画边缘已做 alpha 羽化（素材处理），直接铺放在暖灰底上 */
.error-illustration {
  width: 340px;
  height: auto;
}
...
/* EP 未做全局主题覆盖：主按钮染回品牌蓝，次按钮保持白底灰边 */
.error-btn {
  height: auto;
  margin-left: 0;
  padding: var(--spacing-md) var(--spacing-xxl);
...
.error-btn--ghost {
  ...
  --el-button-active-bg-color: var(--color-bg-hover);
  --el-button-active-border-color: var(--color-primary-dark);
  --el-button-active-text-color: var(--color-primary-dark);
}
</style>
