...
10. **文档入库纪律**（2026-09-21 用户裁决，取代原「`docs/` 整目录忽略」口径）：
    `docs/` 正文、设计素材与测试证据**一律入库**，正常 `git add` 即可，
    不再需要 `-f`（原口径下漏加 `-f` 会静默丢失改动）。
    **仅两类不入库**（`.gitignore` 显式列出）：
    - **生成型大体积产物**：`*.jtl` / `*.jtl.gz` / `jmeter.log` /
      `__pycache__` / `*.pyc` / `.archify-delivery-*/`（单个 `nf_result.jtl`
      即 319 MB，入库会永久膨胀仓库）。例外：`nf_steady_r2.jtl.gz` /
      `nf_steady_r5.jtl` 为 N1 正式性能证据（合计 11 MB，被跟踪文档引用），
      有意保留入库；
    - **第三方资料**：`90_全局/参考资料/`（学校文件、维普操作手册、
      论文格式模版、他人样例开题报告）与 `archive/参考资料归档-*/`
      ——**仓库为公开仓库，入库等于再发布他人版权资料**。
    判断新增文件是否该入库：问「这是本项目自己产出的吗」，
    是则入库；否（他人版权/机器生成的大文件）则补进 `.gitignore` 并注明理由。
...
- **业务范围边界**（2026-09-21 用户裁决，系统定义 v1.5；权威表述见
  `docs/10_系统定义/系统定义.md` §3 非目标）：
  - **出租 = 系统内完整闭环**：房源挂牌 → 展示筛选 → 看房预约 → 入住申请
    与审核 → 租赁协议在线确认 → 租约生效 → 续约支付 → 退租归档；
  - **出售 = 仅房源信息展示 + 线下看房预约入口**：房屋买卖交易环节
    （网签备案 / 资金监管 / 过户登记 / 税费代扣 / 中介佣金撮合）须对接
    不动产登记与银行资金监管系统，超出纯软件系统边界，不介入；
  - 租赁协议的「在线确认留痕」**不构成《电子签名法》意义的可靠电子签名**
    （未接 CA 证书 / 可信时间戳 / 第三方电子签），须随功能如实声明。
- **技术栈与版本约束**（2026-09-06 于 30_系统设计锁定；基线取自已整体
  ...
  - 后端（`backend/`，Maven + mvnw wrapper）：
    ...
    - 支付 SDK（2026-09-20 引入，R61 租约续约支付，见决策日志同日条目）：
      `com.alipay.sdk:alipay-sdk-java 4.40.1006.ALL`（支付宝电脑网站支付
      alipay.trade.page.pay，沙箱个人可开通）+ `com.github.wechatpay-apiv3:
      wechatpay-java 0.2.17`（微信 Native 下单 v3，无公开沙箱、需商户凭据）；
      两渠道均**凭据门控**——凭据（`payment.alipay.*` / `payment.wechat.*`）
      未配置时渠道禁用并明示（GET /payment/channels enabled=false，不报错不阻断）；
      本地无公网回调，支付确认以**后端主动查单 + 前端轮询**替代异步 notify
      （公网部署按官方文档补 notify 回调，PaymentController 注释预留接入点）；
    - Lombok；
...
- **目录结构约定**（基于 30_系统设计/架构设计.md §2）：
  ...
  - **前端目录结构**（`frontend/src/`，基于 30_系统设计/UI设计.md §2）：
    ...
    │   │                     # 口径（2026-09-21 重述，可复现）：views/ 下 91 个
    │   │                     # .vue 扣除 布局壳 4（各端 *Layout.vue）+ 全局错误页 2
    │   │                     # （ForbiddenView/NotFoundView）+ 登录注册 2（auth/）+
    │   │                     # 弹窗/抽屉/面板等子组件 18（*Dialog/*Drawer/*Pane/
    │   │                     # *Panel/*Tab/*Card/*Header/*Step）→ 65
    │   │                     # 分布：管理端 33 / 居民端 22 / 游客端 5 / 服务人员端 5。
    │   │                     # 演进：原 58（2026-09-07 P1 全部实现，时点 64、
    │   │                     # 2026-09-09 冗余清理删 6 页；联调复测 12/12 通过）
    │   │                     # + 2026-09-19/20 批次新增 7 页
    │   │                     # （居民端 LeaseListView / LeaseRenewDialog /
