...
**前端**（已可运行；三端界面 P1 已全部实现——游客端/居民端/服务人员端/管理端
64 个视图 + 12 模块 API 封装，接口联调待后端业务模块完成后进行）：
...
四角色权限 + 反馈附件独立管理）、四个定时任务（租期判定/到期提醒/公告下线/
浏览统计回写，Redisson 锁 + 任务日志）、WebSocket 实时通知（/ws 端点 STOMP，
在线推送 + 轮询兜底）、看房可约时段接口（GET /housings/{id}/available-slots，
周模板展开 + 占用计数）；缺陷修复：公告删除 FK 冲突（BE-ISSUE-8）等。
